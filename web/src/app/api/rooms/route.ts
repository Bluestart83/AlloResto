...
import { DiningRoom } from "@/db/entities/DiningRoom";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const rooms = await ds.getRepository(DiningRoom).find({
      where: { restaurantId },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const repo = ds.getRepository(DiningRoom);
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    await ds.getRepository(DiningRoom).update(body.id, updates);
    const updated = await ds.getRepository(DiningRoom).findOne({
      where: { id: body.id },
      relations: ["tables"],
    });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await ds.getRepository(DiningRoom).delete(id);
