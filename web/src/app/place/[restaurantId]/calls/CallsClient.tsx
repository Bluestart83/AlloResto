...
import { useState, useEffect, useCallback, Fragment, useTransition } from "react";
...
export default function CallsClient({ restaurantId, initialCalls }: CallsClientProps) {
  ...
  const reload = () => {
    startTransition(async () => {
      const data = await fetchCalls(restaurantId);
  ...
  useEffect(() => {
    fetchBlockedPhones();
    // Fetch restaurant display currency
  ...
  }, [restaurantId, fetchBlockedPhones, billingCurrency]);
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <button className="btn btn-outline-secondary btn-sm" onClick={reload} disabled={isPending}>
          <i className={`bi bi-arrow-clockwise me-1 ${isPending ? "spin" : ""}`}></i>
          Rafraîchir
        </button>
      </div>
